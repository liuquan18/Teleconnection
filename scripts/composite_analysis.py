...
# spatial patterns
## Z200
sept.visu_composite_spa(ind,plev = 20000)
sept.visu_composite_spa(dep,plev = 20000)

## Z1000
sept.visu_composite_spa(ind,plev = 100000)
sept.visu_composite_spa(dep,plev = 100000)

#%%
# diff between pos and neg
ind_extr_diff = ind.sel(extr_type = 'pos')+ind.sel(extr_type = 'neg')
sept.visu_eofspa(ind_extr_diff,
plev = [20000,100000], levels = np.arange(-0.5,0.54,0.1))

dep_extr_diff = dep.sel(extr_type = 'pos')+dep.sel(extr_type = 'neg')
sept.visu_eofspa(dep_extr_diff,
plev = [20000,100000], levels = np.arange(-0.5,0.54,0.1))

#%%
# diff between diff
common_diff = ind_extr_diff-dep_extr_diff
sept.visu_eofspa(common_diff,plev=[20000,100000],levels = np.arange(-0.3,0.31,0.05))
